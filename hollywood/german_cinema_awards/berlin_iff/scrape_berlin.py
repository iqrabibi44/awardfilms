...
BERLIN_URLS = {
    "Golden Bear": "https://en.wikipedia.org/wiki/Golden_Bear",
    "Silver Bear Best Director": "https://en.wikipedia.org/wiki/Silver_Bear_for_Best_Director",
    "Silver Bear Best Actor": "https://en.wikipedia.org/wiki/Silver_Bear_for_Best_Actor",
    "Silver Bear Best Actress": "https://en.wikipedia.org/wiki/Silver_Bear_for_Best_Actress",
    "Silver Bear Best Screenplay": "https://en.wikipedia.org/wiki/Silver_Bear_for_Best_Screenplay",
    "Silver Bear Grand Jury Prize": "https://en.wikipedia.org/wiki/Silver_Bear_Grand_Jury_Prize"
}
...
def parse_berlin_table(table, category, source_url):
    records = []
    ...
    for row in rows:
        ...
        # In Berlin tables, mostly all listed are winners unless there's a specific background color denoting it
        is_winner = True
        ...
        if headers:
            ...
            for ck in ["country", "country of origin"]:
                if ck in col_map:
                    country = col_map[ck]
                    break
        ...
        if film or nominee:
            ...
            ceremony_name = f"Berlin IFF {year}"
            records.append({
                "year": year,
    ...
    for table in tables:
        ...
        if first_row:
            ...
            if any(h in ["film", "actor", "actress", "director", "winner", "year"] for h in headers):
                records.extend(parse_berlin_table(table, category, url))
            else:
...
def main():
    print("[*] Starting Berlin IFF scraping...")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        future_to_cat = {
            ...
            for i, (cat, url) in enumerate(BERLIN_URLS.items())
        }
    ...
    if not df.empty:
        ...
        output_path = os.path.join(output_dir, "berlin_awards.csv")
        df.to_csv(output_path, index=False, quoting=csv.QUOTE_MINIMAL)
        print(f"\n[+] Berlin data saved: {output_path}")
