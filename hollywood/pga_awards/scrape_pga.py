...
URL = "https://en.wikipedia.org/wiki/Producers_Guild_of_America_Awards"
        ...
        if parent_td:
            for cat_elem in parent_td.find_all(['b', 'div', 'th']):
                if cat_elem.sourceline is not None and ul.sourceline is not None and cat_elem.sourceline <= ul.sourceline:
                    ...
                    if any(w in text.lower() for w in ['best', 'outstanding', 'award', 'producer', 'feature', 'film', 'television', 'comedy', 'drama', 'series', 'motion picture']):
                        category = text
                        break
...
def main():
    ...
    ceremony_links = []
    # Match both standard PGA and older Golden Laurel awards naming conventions
    for a in soup.find_all('a', href=re.compile(r'/wiki/\d+(st|nd|rd|th)_(Producers_Guild_of_America|Golden_Laurel)_Awards')):
        ...
        if year_match:
            ...
            # 1st PGA/Golden Laurel Awards was held in 1990
            year = 1989 + iteration
    ...
    out_file = os.path.join(out_dir, "pga_awards.csv")
