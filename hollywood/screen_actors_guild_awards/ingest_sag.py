...
def determine_dept(category_name):
    ...
    if "actor" in cat_lower or "actress" in cat_lower or "star" in cat_lower or "performance" in cat_lower or "role" in cat_lower or "cast" in cat_lower:
        return "Acting"
    if "stunt" in cat_lower or "ensemble" in cat_lower:
        return "Acting"
    ...
    if "screenplay" in cat_lower or "writing" in cat_lower:
        return "Writing"
    if "television" in cat_lower or "series" in cat_lower or "comedy" in cat_lower or "drama" in cat_lower:
        return "Television"
    return "Other"

def main():
    conn = get_db()
    # To dramatically speed up inserts, we disable autocommit and commit in chunks
    conn.autocommit = False
    
    csv_path = os.path.join(os.path.dirname(__file__), "screen_actors_guild_awards.csv")
    ...
    ceremony_id = get_ceremony_id(conn, "screen-actors-guild-awards")
    ...
    print(f"[*] Processing {len(df)} records for Screen Actors Guild Awards...")
    for idx, row in df.iterrows():
        ...
        if year not in edition_cache:
            edition_cache[year] = upsert_edition(conn, ceremony_id, year, 1995, "sag-awards")
        edition_id = edition_cache[year]
        
        # Category
        cat_slug = f"sag-awards-{slugify(cat_name)}"
        ...
        film_id = None
        if film_title:
            # We assume country USA, language English for SAG mostly
