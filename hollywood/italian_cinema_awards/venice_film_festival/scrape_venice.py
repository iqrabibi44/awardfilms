...
VENICE_URLS = {
    "Golden Lion": "https://en.wikipedia.org/wiki/Golden_Lion",
    "Silver Lion": "https://en.wikipedia.org/wiki/Silver_Lion",
    "Volpi Cup for Best Actor": "https://en.wikipedia.org/wiki/Volpi_Cup_for_Best_Actor",
    "Volpi Cup for Best Actress": "https://en.wikipedia.org/wiki/Volpi_Cup_for_Best_Actress"
}
...
def parse_venice_table(table, category, source_url):
    records = []
    ...
    for row in rows:
        ...
        # For Venice, the tables are mostly just the winners. We assume is_winner=True for all listed.
        is_winner = True
        ...
        if headers:
            ...
            # Find nominee
            for nk in ["director", "actor", "actress", "winner(s)"]:
                if nk in col_map:
                    nominee = col_map[nk]
                    break
        ...
        if film or nominee:
            ...
            ceremony_name = f"Venice Film Festival {year}"
            records.append({
                "year": year,
...
def parse_page(url, category, thread_id):
    ...
    for table in tables:
        ...
        if first_row:
            ...
            if any(h in ["film", "actor", "actress", "director", "winner(s)", "year"] for h in headers):
                records.extend(parse_venice_table(table, category, url))
            else:
...
def main():
    print("[*] Starting Venice Film Festival scraping...")
    
    all_records = []
    
    with ThreadPoolExecutor(max_workers=4) as executor:
        future_to_cat = {
            executor.submit(parse_page, url, cat, i % 4): cat
            for i, (cat, url) in enumerate(VENICE_URLS.items())
        }
    ...
    if not df.empty:
        ...
        output_path = os.path.join(output_dir, "venice_awards.csv")
        df.to_csv(output_path, index=False, quoting=csv.QUOTE_MINIMAL)
        print(f"\n[+] Venice data saved: {output_path}")
