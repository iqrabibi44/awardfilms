...
DAVID_URLS = {
    "Best Film": "https://en.wikipedia.org/wiki/David_di_Donatello_for_Best_Film",
    "Best Director": "https://en.wikipedia.org/wiki/David_di_Donatello_for_Best_Director",
    "Best Actor": "https://en.wikipedia.org/wiki/David_di_Donatello_for_Best_Actor",
    "Best Actress": "https://en.wikipedia.org/wiki/David_di_Donatello_for_Best_Actress",
    "Best Supporting Actor": "https://en.wikipedia.org/wiki/David_di_Donatello_for_Best_Supporting_Actor",
    "Best Supporting Actress": "https://en.wikipedia.org/wiki/David_di_Donatello_for_Best_Supporting_Actress",
    "Best Foreign Film": "https://en.wikipedia.org/wiki/David_di_Donatello_for_Best_Foreign_Film"
}
...
def parse_david_table(table, category, source_url):
    records = []
    ...
    for row in rows:
        ...
        if headers:
            ...
            # Find film
            for fk in ["film", "title", "motion picture", "english title", "original title"]:
                if fk in col_map:
                    film = col_map[fk]
                    break
            
            # Find nominee
            for nk in ["director", "actor", "actress", "nominee", "recipient"]:
                if nk in col_map:
                    nominee = col_map[nk]
                    break
        ...
        if film or nominee:
            ...
            if not country: country = "Italy" if "foreign" not in category.lower() else ""
            ceremony_name = f"David di Donatello {year}"
            records.append({
                "year": year,
    ...
    for table in tables:
        ...
        if first_row:
            ...
            if any(h in ["film", "actor", "actress", "director", "recipient", "nominee", "year"] for h in headers):
                records.extend(parse_david_table(table, category, url))
            else:
...
def main():
    print("[*] Starting David di Donatello scraping...")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        future_to_cat = {
            ...
            for i, (cat, url) in enumerate(DAVID_URLS.items())
        }
    ...
    if not df.empty:
        ...
        output_path = os.path.join(output_dir, "david_awards.csv")
        df.to_csv(output_path, index=False, quoting=csv.QUOTE_MINIMAL)
        print(f"\n[+] David data saved: {output_path}")
