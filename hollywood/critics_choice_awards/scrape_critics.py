...
URL = "https://en.wikipedia.org/wiki/Critics%27_Choice_Awards"
        ...
        if parent_td:
            for cat_elem in parent_td.find_all(['b', 'div', 'th']):
                if cat_elem.sourceline is not None and ul.sourceline is not None and cat_elem.sourceline <= ul.sourceline:
                    ...
                    if any(w in text.lower() for w in ['best', 'outstanding', 'award', 'feature', 'film', 'television', 'comedy', 'drama', 'series', 'performance', 'screenplay', 'director', 'movie']):
                        category = text
                        break
...
def main():
    ...
    ceremony_links = []
    for a in soup.find_all('a', href=re.compile(r'/wiki/\d+(st|nd|rd|th)_Critics\x27_Choice')):
        ...
        if year_match:
            ...
            # 1st Critics' Choice Awards was held in 1996
            year = 1995 + iteration
    ...
    out_file = os.path.join(out_dir, "critics_choice_awards.csv")
