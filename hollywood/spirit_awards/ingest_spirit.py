...
def determine_dept(category_name):
    ...
    if 'screenplay' in cat_lower or 'writer' in cat_lower or 'writing' in cat_lower:
        return 'Writing'
    if 'actor' in cat_lower or 'actress' in cat_lower or 'performance' in cat_lower or 'supporting' in cat_lower or 'lead' in cat_lower:
        return 'Acting'
...
def main():
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "spirit_awards.csv")
    ...
    ceremony_id = get_ceremony_id(conn, "independent-spirit-awards")
    
    total_nominations = 0
    edition_cache = {}
    category_cache = {}
    
    print(f"[*] Processing {len(df)} records for Independent Spirit Awards...")
    for idx, row in df.iterrows():
        ...
        if year not in edition_cache:
            edition_cache[year] = upsert_edition(conn, ceremony_id, year, 1986, "independent-spirit-awards")
        edition_id = edition_cache[year]
        
        cat_slug = f"independent-spirit-awards-{slugify(cat_name)}"
