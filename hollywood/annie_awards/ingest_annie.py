...
def determine_dept(category_name):
    ...
    if 'screenplay' in cat_lower or 'writer' in cat_lower or 'writing' in cat_lower:
        return 'Writing'
    if 'voice' in cat_lower or 'actor' in cat_lower or 'actress' in cat_lower:
        return 'Acting'
    if 'music' in cat_lower or 'score' in cat_lower or 'song' in cat_lower:
        return 'Music'
    if 'production design' in cat_lower or 'storyboarding' in cat_lower or 'character design' in cat_lower or 'character animation' in cat_lower or 'effects animation' in cat_lower or 'animated effects' in cat_lower or 'editorial' in cat_lower:
        return 'Production'
    return "General"
...
def main():
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "annie_awards.csv")
    ...
    ceremony_id = get_ceremony_id(conn, "annie-awards")
    
    total_nominations = 0
    edition_cache = {}
    category_cache = {}
    
    print(f"[*] Processing {len(df)} records for Annie Awards...")
    for idx, row in df.iterrows():
        ...
        if year not in edition_cache:
            edition_cache[year] = upsert_edition(conn, ceremony_id, year, 1972, "annie-awards")
        edition_id = edition_cache[year]
        
        cat_slug = f"annie-awards-{slugify(cat_name)}"
