...
def main():
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "wga_awards.csv")
    ...
    ceremony_id = get_ceremony_id(conn, "wga-awards")
    
    total_nominations = 0
    edition_cache = {}
    category_cache = {}
    
    print(f"[*] Processing {len(df)} records for Writers Guild of America Awards...")
    for idx, row in df.iterrows():
        ...
        if year not in edition_cache:
            edition_cache[year] = upsert_edition(conn, ceremony_id, year, 1949, "wga-awards")
        edition_id = edition_cache[year]
        
        cat_slug = f"wga-awards-{slugify(cat_name)}"
