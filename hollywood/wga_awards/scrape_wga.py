        ...
        if parent_td:
            for cat_elem in parent_td.find_all(['b', 'div', 'th']):
                if cat_elem.sourceline is not None and ul.sourceline is not None and cat_elem.sourceline <= ul.sourceline:
                    ...
                    if any(w in text.lower() for w in ['best', 'outstanding', 'award', 'screenplay', 'write', 'feature', 'film', 'television', 'comedy', 'drama', 'series']):
                        category = text
                        break
...
def main():
    ...
    ceremony_links = []
    for a in soup.find_all('a', href=re.compile(r'/wiki/\d+(st|nd|rd|th)_Writers_Guild_of_America_Awards')):
        ...
        if year_match:
            ...
            # 1st WGA Awards was held in 1949
            year = 1948 + iteration
    ...
    out_file = os.path.join(out_dir, "wga_awards.csv")
