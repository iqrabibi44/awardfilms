...
def determine_dept(category_name):
    ...
    if 'screenplay' in cat_lower or 'writer' in cat_lower or 'writing' in cat_lower:
        return 'Writing'
    if 'actor' in cat_lower or 'actress' in cat_lower or 'performance' in cat_lower or 'supporting' in cat_lower or 'lead' in cat_lower or 'guest' in cat_lower or 'young' in cat_lower:
        return 'Acting'
    ...
    if 'music' in cat_lower or 'score' in cat_lower or 'song' in cat_lower:
        return 'Music'
    if 'costume' in cat_lower or 'makeup' in cat_lower or 'special effects' in cat_lower or 'visual effects' in cat_lower or 'production design' in cat_lower:
        return 'Production'
    return "General"
...
def main():
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "saturn_awards.csv")
    ...
    ceremony_id = get_ceremony_id(conn, "saturn-awards")
    
    total_nominations = 0
    edition_cache = {}
    category_cache = {}
    
    print(f"[*] Processing {len(df)} records for Saturn Awards...")
    for idx, row in df.iterrows():
        ...
        if year not in edition_cache:
            edition_cache[year] = upsert_edition(conn, ceremony_id, year, 1973, "saturn-awards")
        edition_id = edition_cache[year]
        
        cat_slug = f"saturn-awards-{slugify(cat_name)}"
