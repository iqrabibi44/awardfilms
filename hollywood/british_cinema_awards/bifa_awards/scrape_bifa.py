...
BIFA_URLS = {
    "Best British Independent Film": "https://en.wikipedia.org/wiki/BIFA_Award_for_Best_British_Independent_Film",
    "Best Director": "https://en.wikipedia.org/wiki/BIFA_Award_for_Best_Director",
    "Best Actor": "https://en.wikipedia.org/wiki/BIFA_Award_for_Best_Performance_by_an_Actor_in_a_British_Independent_Film",
    "Best Actress": "https://en.wikipedia.org/wiki/BIFA_Award_for_Best_Performance_by_an_Actress_in_a_British_Independent_Film",
    "Best Supporting Actor": "https://en.wikipedia.org/wiki/BIFA_Award_for_Best_Supporting_Actor",
    "Best Supporting Actress": "https://en.wikipedia.org/wiki/BIFA_Award_for_Best_Supporting_Actress",
    "Best Screenplay": "https://en.wikipedia.org/wiki/BIFA_Award_for_Best_Screenplay"
}
...
def parse_bifa_table(table, category, source_url):
    records = []
    ...
    for table in tables:
        ...
        if first_row:
            ...
            if any(h in ["film", "actor", "actress", "director", "recipient", "nominee", "year"] for h in headers):
                records.extend(parse_bifa_table(table, category, url))

    return records

def main():
    print("[*] Starting BIFA scraping...")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        future_to_cat = {
            ...
            for i, (cat, url) in enumerate(BIFA_URLS.items())
        }
    ...
    if not df.empty:
        ...
        output_path = os.path.join(output_dir, "bifa_awards.csv")
        df.to_csv(output_path, index=False, quoting=csv.QUOTE_MINIMAL)
        print(f"\n[+] BIFA data saved: {output_path}")
