"""
scrape_siima_kannada.py
Scrapes SIIMA (South Indian International Movie Awards) Kannada-wing categories.
Mirrors scrape_siima.py (Telugu) / scrape_siima_tamil.py / scrape_siima_malayalam.py -
same category-pages-first, edition-pages-fallback pattern.
"""
...
CATEGORIES = [
    ("SIIMA Award for Best Film – Kannada", "Best Film", False),
    ("SIIMA Award for Best Actor – Kannada", "Best Actor", True),
    ("SIIMA Award for Best Actress – Kannada", "Best Actress", True),
    ("SIIMA Award for Best Director – Kannada", "Best Director", False),
    ("SIIMA Award for Best Supporting Actor – Kannada", "Best Supporting Actor", True),
    ("SIIMA Award for Best Supporting Actress – Kannada", "Best Supporting Actress", True),
    ("SIIMA Award for Best Music Director – Kannada", "Best Music Director", False),
]
...
def scrape_editions():
    """
    Fallback: parse per-year SIIMA edition pages. These mix all four languages
    (Telugu/Tamil/Kannada/Malayalam) in one table, so rows are tagged
    '[VERIFY: Kannada-only]' for manual filtering.
    """
    rows = []
    gaps = []
    for page_title, year in EDITION_PAGES.items():
        ...
        for table in tables:
            for tr in table.find_all("tr"):
                ...
                winner_cell = cells[1].get_text(" ", strip=True)
                ...
                rows.append([
                    ...
                    category + " [VERIFY: Kannada-only - edition pages mix languages]",
...
if __name__ == "__main__":
    ...
    write_csv(all_rows, "output/siima_kannada.csv")
    write_gaps(all_gaps, "output/siima_kannada_gaps.csv")
