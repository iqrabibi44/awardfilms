...
urls = {
    "Best Film": "https://es.wikipedia.org/wiki/Anexo:Premio_C%C3%B3ndor_de_Plata_a_la_mejor_pel%C3%ADcula",
    "Best Director": "https://es.wikipedia.org/wiki/Anexo:Premio_C%C3%B3ndor_de_Plata_al_mejor_director",
    "Best Actor": "https://es.wikipedia.org/wiki/Anexo:Premio_C%C3%B3ndor_de_Plata_al_mejor_actor",
    "Best Actress": "https://es.wikipedia.org/wiki/Anexo:Premio_C%C3%B3ndor_de_Plata_a_la_mejor_actriz"
}
...
for category_name, url in urls.items():
    ...
    for t_idx, t in enumerate(tables):
        ...
        # Check if this is a valid award list table
        if not any(h in headers for h in ['año', 'película', 'dirección', 'actor', 'actriz']):
            continue
        ...
        for idx, h in enumerate(headers):
            if 'año' in h or 'cerim' in h:
                year_idx = idx
            elif 'película' in h or 'obra' in h:
                film_idx = idx
            elif 'dirección' in h or 'director' in h or 'actor' in h or 'actriz' in h:
                nominee_idx = idx
        ...
        for r_idx in range(1, len(grid)):
            ...
            # Detect Win
            is_winner = False
            winner_colors = ["#faeb86", "#b0c4de", "yellow", "gold"]
            
            # Check row-level style
            r_style = row_styles[r_idx].lower()
            if any(color in r_style for color in winner_colors):
                is_winner = True
            else:
                # Check cell-level style
...
# Save to CSV
output_path = r"c:\Users\INFOTECH\OneDrive\Desktop\Awardfilms\scripts\condor_de_plata_raw.csv"
