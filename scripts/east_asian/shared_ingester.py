"""
scripts/east_asian/shared_ingester.py
Shared logic for East Asian cinema Wikipedia scraping (MySQL version).
"""
import os
import sys
import re
...
def upsert_category(conn, ceremony_id, slug, name, dept):
    is_craft = dept not in ("Acting", "Directing", "Writing", "Television", "Other")
    cur = conn.cursor()
    try:
        cur.execute(
            """
...
def parse_standard_wikitable(html):
    ...
    for table in soup.find_all("table", class_=re.compile("wikitable")):
        ...
        film_col = next((i for i, h in enumerate(headers) if h in ("film", "films", "movie", "english title")), None)
        ...
        for row in rows[1:]:
            ...
            is_winner = any(c in cls for c in ("success", "yes", "winner", "blue"))
