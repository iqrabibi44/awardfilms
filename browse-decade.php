...
$decadeParam = $_GET['decade'] ?? '2020s';
$decadeNum = (int)$decadeParam;
...
$films = getFilmsByDecade($startYear, $endYear, $limit, $offset);
$hasMore = count($films) === $limit;

$pageTitle = "Award-Winning Films of the " . htmlspecialchars($decadeParam) . " â€” AwardFilms";
...
<div class="min-h-screen bg-[var(--ivory)] pb-20">
    <div style="background: var(--ivory-deep); border-bottom: 1px solid var(--glass-border); padding: 52px 24px 40px;" class="text-start">
        <div class="max-w-[1280px] mx-auto">
            <nav style="display: flex; gap: 8px; font-size: 13px; color: var(--navy-light); margin-bottom: 20px;">
                ...
                <span>/</span>
                <span style="color: var(--gold);">Decade: <?php echo htmlspecialchars($decadeParam); ?></span>
            </nav>
            <h1 style="font-size: clamp(32px, 5vw, 56px); font-family: 'Playfair Display', serif; font-weight: 700; color: var(--navy); margin: 0 0 12px;">
                🏆 Films of the <span class="gold-italic"><?php echo htmlspecialchars($decadeParam); ?></span>
            </h1>
            <p class="font-sans text-[15px] text-[var(--navy-light)] margin-0">
                Explore the top award-winning films released between <?php echo $startYear; ?> and <?php echo $endYear; ?>.
            </p>
        </div>
    </div>
    ...
    <div class="max-w-[1280px] mx-auto px-6 py-12 text-start">
        <?php if (count($films) === 0): ?>
            <p class="text-sm italic text-gray-400">No films found in this decade.</p>
        <?php else: ?>
            ...
            <div style="display: flex; justify-content: center; gap: 12px; margin-top: 52px;">
                <?php if ($currentPage > 1): ?>
                    <a href="/browse/decade/<?php echo $decadeParam; ?>?page=<?php echo $currentPage - 1; ?>" class="btn-secondary">
                        &larr; Previous
                    </a>
                <?php endif; ?>
                <?php if ($hasMore): ?>
                    <a href="/browse/decade/<?php echo $decadeParam; ?>?page=<?php echo $currentPage + 1; ?>" class="btn-accent">
                        Next &rarr;
                    </a>
                <?php endif; ?>
            </div>
